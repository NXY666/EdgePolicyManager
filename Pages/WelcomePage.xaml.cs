    ...
    private async void VersionButton_OnClick(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            var task2 = Task.Run(() =>
            {
                ...
                try
                {
                    var request = new HttpRequestMessage(HttpMethod.Get, "https://20.205.243.168/repos/NXY666/EdgePolicyManager/releases/latest");
            ...
            var task5 = Task.Run(() =>
            {
                ...
                string[] ipList = ["185.199.108.133", "185.199.109.133", "185.199.110.133", "185.199.111.133"];

                foreach (var ip in ipList)
                {
                    try
                    {
                        using var request = new HttpRequestMessage(HttpMethod.Get, $"https://{ip}/NXY666/EdgePolicyManager/master/StaticModels/Policy/SUPPORT_VERSION");
                        request.Headers.Host = "raw.githubusercontent.com";
                    ...
                    {
                        // Ignore the exception and try the next IP
                    }
                }

                // If all IPs fail, use the last one
                return ResourceUtil.GetString("WelcomePage/VersionButton_OnClick/VersionInfoDialog/UnknownText");
            });
